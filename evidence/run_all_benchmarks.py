...
def run_benchmark(
    name: str, script_path: Path, output_file: str, verbose: bool = False
) -> Dict[str, Any]:
    ...
    try:
        ...
        result = subprocess.run(
            cmd, capture_output=not verbose, text=True, cwd=script_path.parent
        )
...
def extract_summary(benchmark_name: str, results: Dict) -> Dict[str, Any]:
    ...
    if "vulnerability" in benchmark_name.lower():
        ...
        summary = results.get("summary", {})
        return {
            "average_token_savings_percentage": summary.get(
                "average_token_savings_percentage"
            ),
...
def run_demo(verbose: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"name": "Refactoring Demo", "status": "error", "error": str(e)}


def generate_summary_report(
    benchmark_results: list, demo_result: Dict
) -> Dict[str, Any]:
    ...
    return {
        ...
        "overall_status": (
            "pass"
            if all(r["status"] == "success" for r in benchmark_results)
            else "partial"
        ),
        "benchmarks_run": len(benchmark_results),
        "benchmarks_passed": sum(
            1 for r in benchmark_results if r["status"] == "success"
        ),
        "total_execution_time_seconds": sum(
            r.get("execution_time_seconds", 0) for r in benchmark_results
        ),
        ...
        "claims_summary": {
            "vulnerability_detection": next(
                (
                    ...
                    if "vulnerability" in r["name"].lower()
                ),
                {},
            ),
...
def print_final_summary(report: Dict[str, Any]):
    ...
    if claims.get("token_efficiency"):
        ...
        print(
            f"  Average savings: {te.get('average_token_savings_percentage', 'N/A')}%"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run all Code Scalpel evidence benchmarks"
    )
    parser.add_argument(
        "--quick", action="store_true", help="Run quick version with fewer iterations"
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    parser.add_argument(
        "--skip-comparison",
        action="store_true",
        help="Skip tool comparison (requires external tools)",
    )
