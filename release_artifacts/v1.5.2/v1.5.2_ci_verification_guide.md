...
|----------|---------|--------|-------|
| **Local Development** | `pytest tests/test_osv_client.py` | [COMPLETE] WORKS | Individual test file |
| **Feature Branches** | `pytest tests/test_osv_client.py tests/test_scan_dependencies.py` | [COMPLETE] WORKS | Paired execution |
| **PR/Merge Checks** | `pytest tests/ -k "not (agents or call_graph)"` | [COMPLETE] WORKS | Exclude problematic paths |
| **Full Suite** | `pytest tests/` | [WARNING] KNOWN ISSUE | 30 test errors (infrastructure only) |
...
jobs:
  test:
    ...
    steps:
      ...
      # [COMPLETE] RECOMMENDED: Use grouped execution instead of full suite
      - name: Run test suite (grouped)
        run: pytest tests/ -k "not (agents or call_graph)" -v --tb=short
      
      # [FAILED] KNOWN ISSUE: Full suite execution may fail
      # run: pytest tests/ -v
...
- [COMPLETE] Fast execution
- [COMPLETE] Clear feedback on failures
- [COMPLETE] Supports CI parallelization
...
- [COMPLETE] Comprehensive coverage (most tests)
- [COMPLETE] Easy to understand

**Limitations:**
- [WARNING] May exclude some unrelated tests
...
**Advantages:**
- [COMPLETE] Maximum isolation
- [COMPLETE] Useful for TDD

**Limitations:**
- [FAILED] Very slow for full suite
...
**[COMPLETE] Production Code Tests Pass:**
```
...
**[COMPLETE] Infrastructure Tests Pass (Isolated):**
```
...
**[WARNING] Full Suite Shows Errors:**
```
...
**Common Issues:**
1. Missing `conftest.py` → Copy from release
2. Import errors → Check Python path
3. Mock setup failed → Check mock library installed
...
echo "All expected tests PASSED [COMPLETE]"
```
...
def main():
    ...
    if passed == total:
        print(f"[COMPLETE] All {total} test groups PASSED")
        return 0
    else:
        print(f"[FAILED] {total - passed} test group(s) FAILED")
        return 1
...
- **v1.5.2:** Released with known limitation, workarounds in place [COMPLETE]
- **v1.5.3:** Root cause investigation and full-suite fix [IN PROGRESS]
- **Post-v1.5.3:** Full suite passes 100% ⏳
