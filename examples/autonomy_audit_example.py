...
def simulate_fix_loop():
    ...
    parent_id = audit.record(
        ...
        output_data={"fixed": True, "changes": "Added missing colon"},
        success=True,
    ...
    child1_id = audit.record(
        ...
        output_data={"cause": "missing colon after if statement", "confidence": 0.95},
        success=True,
        duration_ms=int((time.time() - start_time) * 1000),
        parent_id=parent_id,
    ...
    child2_id = audit.record(
        ...
        input_data={"cause": "missing colon", "line": 42},
        output_data={"fix": "if x == 1:", "diff": "+if x == 1:"},
        success=True,
        duration_ms=int((time.time() - start_time) * 1000),
        parent_id=parent_id,
    )
    ...
    child3_id = audit.record(
        ...
        input_data={"fixed_code": "if x == 1:\n    print('hello')"},
        output_data={"verified": True, "tests_passed": True},
        success=True,
        duration_ms=int((time.time() - start_time) * 1000),
        parent_id=parent_id,
    )
    ...
    parent2_id = audit.record(
        ...
        output_data={"fixed": False, "reason": "Complex type inference required"},
        success=False,
        duration_ms=int((time.time() - start_time) * 1000),
    ...
    for entry in trace:
        ...
        print(
            f"{indent}{status} {entry.event_type}: {entry.operation} ({entry.duration_ms}ms)"
        )
...
def demonstrate_filtering():
    ...
    combined = audit.export(
        format="json", event_types=["ERROR_ANALYSIS"], success_only=True
    )
...
def demonstrate_compliance_reporting():
    ...
    print(
        f"   - Success Rate: {summary['successful'] / summary['total_operations'] * 100:.1f}%"
    )
