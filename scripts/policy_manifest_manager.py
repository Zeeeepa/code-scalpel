...
def cmd_sign(args):
    ...
    if not policy_files:
        ...
        print(f"Looking for: *.yaml, *.yml, *.json, *.rego")
        return 1
    ...
    try:
        ...
        print(f"   3. Ensure agents have SCALPEL_MANIFEST_SECRET set")
...
def cmd_rotate(args):
    ...
    print(f"\n📋 Next steps:")
    print(f"   1. Update SCALPEL_MANIFEST_SECRET everywhere:")
    ...
    print(f"   3. Update CI/CD secrets")
    print(f"   4. Restart all agents")
...
Examples:
  ...
  %(prog)s rotate --policy-dir .code-scalpel --new-secret <new-secret>
    ...
    verify_parser.add_argument("--source", default="file", choices=["file", "git", "env"], help="Manifest source")
    ...
    rotate_parser.add_argument("--old-secret", help="Old signing secret (default: SCALPEL_MANIFEST_SECRET env var)")
