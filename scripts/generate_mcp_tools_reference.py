    ...
    for tool in tools_result.tools:
        ...
        tools.append(
            ToolDoc(
                ...
                input_schema=(
                    _as_dict(input_schema) if input_schema is not None else None
                ),
            )
        )
...
def _write_markdown(out_path: Path, tools: list[ToolDoc]) -> None:
    ...
    lines.append("")
    lines.append("- Generated by: `scripts/generate_mcp_tools_reference.py`")
...
async def main() -> int:
    ...
    tools = await asyncio.wait_for(
        _fetch_tools(project_root), timeout=timedelta(seconds=90).total_seconds()
    )
