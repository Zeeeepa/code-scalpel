...
1. Parse JSON output from rubocop --format=json
2. Execute RuboCop via subprocess/bundler
3. Load configuration from .rubocop.yml files
4. Categorize violations by cop category
5. Apply auto-corrections via rubocop --auto-correct
6. Generate JSON/SARIF/HTML reports
7. Calculate complexity and trend metrics
8. Detect Rails-specific cop violations
"""
...
class RuboCopParser:
    ...
    def detect_rails_specific_issues(
        ...
        """Filter for Rails-specific violations - Phase 2 TODO [20251221_TODO]"""
