...
class ContractBreachDetector:
    ...
    def _detect_format_breach(
        ...
        for field, expected_type in client_expected.items():
            actual_type = server_format.get(field)
            if actual_type and actual_type != expected_type:
                format_mismatches.add(f"{field}: {expected_type} -> {actual_type}")
