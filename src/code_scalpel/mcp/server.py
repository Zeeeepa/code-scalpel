...
def _get_current_tier() -> str:
    ...
    [20260312_FEATURE] Delegates to the canonical ``get_effective_tier()`` in
    ``licensing.authorization`` so that ``_FORCE_ENTERPRISE_MODE``, env-var
    overrides, and JWT license validation are all resolved in ONE place.

    Kept as a thin wrapper so existing callers that import
    ``_get_current_tier`` from this module continue to work unchanged.

    Returns:
        str: One of 'community', 'pro', or 'enterprise'
    """
    # [20260312_FEATURE] Unified delegation — single source of truth
    from code_scalpel.licensing.authorization import get_effective_tier

    return get_effective_tier()
