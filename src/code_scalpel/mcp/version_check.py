...
def check_version_mismatch():
    ...
    if latest_version != __version__:
        ...
        if action == "error":
            ...
            print("Exiting due to CODE_SCALPEL_VERSION_MISMATCH_ACTION=error", file=sys.stderr)
            ...
            import subprocess
            try:
                subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "codescalpel"])
                print("Upgrade successful. Please restart the MCP server.", file=sys.stderr)
                os._exit(0)
