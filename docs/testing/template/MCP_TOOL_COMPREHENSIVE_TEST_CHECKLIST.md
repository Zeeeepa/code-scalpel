# MCP Tool Comprehensive Test Checklist
**Framework Version:** 3.0
**Purpose:** Universal testing checklist for all Code Scalpel MCP tools
...
**TESTING REQUIREMENT: Use real JWT licenses, NOT mocks**
- Test fixtures MUST set `CODE_SCALPEL_LICENSE_PATH` to real license files in `tests/licenses/`
- DO NOT use mock tier objects (`mock_tier_pro`, `mock_tier_enterprise`) - they bypass validation
- Community tier = no license file or unset env var
- Pro tier = `CODE_SCALPEL_LICENSE_PATH` pointing to `tests/licenses/code_scalpel_license_pro_*.jwt`
- Enterprise tier = `CODE_SCALPEL_LICENSE_PATH` pointing to `tests/licenses/code_scalpel_license_enterprise_*.jwt`

### 2.1 Community Tier (No License)
**Purpose:** Verify base functionality without license

**Test Setup:** Unset `CODE_SCALPEL_LICENSE_PATH` or point to nonexistent file
...
**Example Tests:**
```python
def test_community_core_features(monkeypatch):
    """Community tier provides core features."""
    # Ensure no license active
...
def test_community_no_pro_fields(monkeypatch):
...
def test_community_file_size_limit(monkeypatch):
    """Community tier enforces 1MB file size limit."""
    monkeypatch.delenv("CODE_SCALPEL_LICENSE_PATH", raising=False)
    monkeypatch.setenv("CODE_SCALPEL_DISABLE_LICENSE_DISCOVERY", "1")
    
    large_code = "x = 1\n" * 1_000_000  # >1MB
    result = tool.execute(code=large_code)
    assert result.success is False
...
**Test Setup:** Set `CODE_SCALPEL_LICENSE_PATH` to real Pro license JWT file
...
**Example Tests:**
```python
# Fixture for Pro tier in conftest.py
PRO_LICENSE = Path(__file__).parent.parent / "licenses" / "code_scalpel_license_pro_valid.jwt"

@pytest.fixture
def use_pro_tier(monkeypatch):
    """Activate Pro tier with real JWT license."""
    monkeypatch.setenv("CODE_SCALPEL_LICENSE_PATH", str(PRO_LICENSE))
    monkeypatch.setenv("CODE_SCALPEL_DISABLE_LICENSE_DISCOVERY", "1")
    yield

def test_pro_exclusive_features(use_pro_tier):
...
def test_pro_no_enterprise_fields(use_pro_tier):
    """Enterprise fields excluded at Pro tier."""
    result = tool.execute(input)
    assert result.enterprise_only_field == []

def test_pro_increased_limits(use_pro_tier):
    """Pro tier has higher limits than Community."""
    medium_code = "x = 1\n" * 5_000_000  # 5MB
    result = tool.execute(code=medium_code)
    assert result.success is True
```
...
**Test Setup:** Set `CODE_SCALPEL_LICENSE_PATH` to real Enterprise license JWT file
...
**Example Tests:**
```python
# Fixture for Enterprise tier in conftest.py
ENTERPRISE_LICENSE = Path(__file__).parent.parent / "licenses" / "code_scalpel_license_enterprise_valid.jwt"

@pytest.fixture
def use_enterprise_tier(monkeypatch):
    """Activate Enterprise tier with real JWT license."""
    monkeypatch.setenv("CODE_SCALPEL_LICENSE_PATH", str(ENTERPRISE_LICENSE))
    monkeypatch.setenv("CODE_SCALPEL_DISABLE_LICENSE_DISCOVERY", "1")
    yield

def test_enterprise_all_features(use_enterprise_tier):
...
def test_enterprise_unlimited_depth(use_enterprise_tier):
    """Enterprise tier has unlimited depth."""
    result = tool.execute(input, max_depth=1000)
    assert result.success is True
...
**Execution rule:** Use real JWT license artifacts (no mocked tiers). Point `CODE_SCALPEL_LICENSE_PATH` to the fixture license file (e.g., `tests/licenses/code_scalpel_license_pro_*.jwt`) and set `CODE_SCALPEL_DISABLE_LICENSE_DISCOVERY=1` when forcing a specific path; use malformed files to assert fallback.
...
**Example Tests Using Real Licenses:**
```python
import os
import pytest
from pathlib import Path

# Locate real JWT license files
LICENSE_DIR = Path(__file__).parent.parent.parent / "licenses"
PRO_LICENSE = LICENSE_DIR / "code_scalpel_license_pro_20260101_190345.jwt"
ENTERPRISE_LICENSE = LICENSE_DIR / "code_scalpel_license_enterprise_20260101_190754.jwt"

@pytest.fixture
def use_pro_tier(monkeypatch):
    """Activate Pro tier using real JWT license file."""
    monkeypatch.setenv("CODE_SCALPEL_LICENSE_PATH", str(PRO_LICENSE))
    monkeypatch.setenv("CODE_SCALPEL_DISABLE_LICENSE_DISCOVERY", "1")
    yield
    monkeypatch.delenv("CODE_SCALPEL_LICENSE_PATH", raising=False)
...
@pytest.fixture
def use_enterprise_tier(monkeypatch):
    """Activate Enterprise tier using real JWT license file."""
    monkeypatch.setenv("CODE_SCALPEL_LICENSE_PATH", str(ENTERPRISE_LICENSE))
    monkeypatch.setenv("CODE_SCALPEL_DISABLE_LICENSE_DISCOVERY", "1")
    yield
    monkeypatch.delenv("CODE_SCALPEL_LICENSE_PATH", raising=False)
...
def test_valid_pro_license(use_pro_tier):
    """Valid Pro license detected from real JWT file."""
    from code_scalpel.mcp.server import _get_current_tier
    tier = _get_current_tier()
    assert tier == "pro", f"Expected pro tier, got {tier}"

def test_expired_license_fallback(monkeypatch, tmp_path):
    """Expired/invalid license falls back to Community."""
    malformed_jwt = tmp_path / "broken.jwt"
    malformed_jwt.write_text("not.a.valid.jwt")
    
    monkeypatch.setenv("CODE_SCALPEL_LICENSE_PATH", str(malformed_jwt))
    monkeypatch.setenv("CODE_SCALPEL_DISABLE_LICENSE_DISCOVERY", "1")
    
    from code_scalpel.mcp.server import _get_current_tier
    tier = _get_current_tier()
    assert tier == "community", f"Expected community fallback, got {tier}"

def test_missing_license_defaults_to_community(monkeypatch):
...
**❌ DO NOT use mock tier objects** - they bypass JWT validation:
```python
# BAD - Doesn't test real license validation
@pytest.fixture
def mock_tier_pro():
    return MagicMock(tier="pro")
```
