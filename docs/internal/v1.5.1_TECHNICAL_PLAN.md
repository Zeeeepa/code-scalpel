...
**The Problem:**
- v1.5.0 `security_scan` on `models.py` will find the vulnerability ✅
- But `security_scan` on `views.py` alone won't detect that `user_id` is tainted ❌
...
- ❌ `importlib.import_module()` (dynamic, can't resolve statically)
- ❌ `__import__()` (dynamic)
- ❌ `sys.path` manipulation (too complex)
...
```
Week 1 (Jan 6-10)     Phase 1: Import Resolution ✓
                      - ImportGraph class
                      - File crawling & parsing
                      - 40+ tests

Week 2 (Jan 13-17)    Phase 2: Cross-File Extraction ✓
                      - CallGraphAnalyzer
                      - MCP tool integration
                      - 30+ tests

Week 3 (Jan 20-24)    Phase 3: Cross-File Taint ✓
                      - Inter-procedural analysis
