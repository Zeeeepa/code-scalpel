...
class TestCapabilitiesWithProLicense:
    ...
    def test_pro_tier_shows_pro_tools(self, clear_all_caches, pro_license_path):
        ...
        try:
            ...
            available_tools = sum(1 for tool in capabilities.values() if tool.get("available", False))
            assert available_tools >= 19, f"Pro tier should have 19+ available tools, got {available_tools}"
        finally:
    ...
    def test_pro_tier_locks_enterprise_tools(self, clear_all_caches, pro_license_path):
        ...
        try:
            ...
            locked_tools = [tool_id for tool_id, cap in capabilities.items() if not cap.get("available", False)]
            ...
            assert len(locked_tools) == 3, (
                f"Pro tier should lock exactly 3 tools, got {len(locked_tools)}: " f"{locked_tools}"
            )
        finally:
            os.environ.pop("CODE_SCALPEL_LICENSE_PATH", None)
...
class TestCapabilitiesWithEnterpriseLicense:
    ...
    def test_enterprise_tier_shows_all_tools(self, clear_all_caches, enterprise_license_path):
        ...
        try:
            ...
            available_tools = sum(1 for tool in capabilities.values() if tool.get("available", False))
            assert available_tools == 10, f"Enterprise tier should have 10 available tools, got {available_tools}"
        finally:
            os.environ.pop("CODE_SCALPEL_LICENSE_PATH", None)

    def test_enterprise_has_focused_tool_set(self, clear_all_caches, enterprise_license_path):
        ...
        try:
            ...
            enterprise_tools = {
                tool_id for tool_id, cap in enterprise_capabilities.items() if cap.get("available", False)
            }
    ...
    def test_enterprise_has_different_tools_than_pro(self, clear_all_caches, pro_license_path, enterprise_license_path):
        ...
        try:
            ...
            assert pro_cap.get("available", False), "Pro tier should have get_file_context available"
            ...
            assert not enterprise_cap.get("available", False), (
                "Enterprise tier doesn't have get_file_context " "(tiers have different tools, not hierarchical)"
            )

            # But Enterprise should have other tools
            enterprise_analyze = get_tool_capabilities("analyze_code", "enterprise")
            assert enterprise_analyze.get("available", False), "Enterprise tier should have analyze_code available"
        finally:
            pass
...
class TestCapabilitiesWithoutLicense:
    ...
    def test_community_tier_has_basic_tools(self, clear_all_caches):
        ...
        available_tools = sum(1 for tool in capabilities.values() if tool.get("available", False))
        assert available_tools == 22, f"Community tier should have all 22 tools, got {available_tools}"
    ...
    def test_all_22_tools_listed(self, clear_all_caches):
        ...
        assert len(capabilities) == 22, f"Expected 22 tools total, got {len(capabilities)}"
