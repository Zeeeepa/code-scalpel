...
'''
    ...
    def test_generate_constraint_spec_tier_limits(self, sample_python_file):
        """Test that tier limits are applied."""
        # Community tier
        spec_community = self.generator.generate_constraint_spec(
            file_path=sample_python_file,
            instruction="Test",
            tier="community",
        )
        assert spec_community.tier == "community"

        # Pro tier
        spec_pro = self.generator.generate_constraint_spec(
            file_path=sample_python_file,
            instruction="Test",
            tier="pro",
        ...
        assert spec_pro.tier == "pro"

        # Enterprise tier
        spec_enterprise = self.generator.generate_constraint_spec(
            file_path=sample_python_file,
            instruction="Test",
            tier="enterprise",
        )
        assert spec_enterprise.tier == "enterprise"
    ...
    def test_get_tier_graph_depth(self):
        """Test tier-based graph depth limits."""
        assert SpecGenerator._get_tier_graph_depth("community") == 2
        assert SpecGenerator._get_tier_graph_depth("pro") == 5
        assert SpecGenerator._get_tier_graph_depth("enterprise") == 999
        assert SpecGenerator._get_tier_graph_depth("unknown") == 2  # Default

    def test_get_tier_context_lines(self):
        """Test tier-based context line limits."""
        assert SpecGenerator._get_tier_context_lines("community") == 100
        assert SpecGenerator._get_tier_context_lines("pro") == 200
        assert SpecGenerator._get_tier_context_lines("enterprise") == 999
        assert SpecGenerator._get_tier_context_lines("unknown") == 100  # Default
    ...
    def test_generate_spec_with_zero_context_limit(self, sample_python_file):
    ...
    def test_markdown_includes_tier_label(self):
        """Test that markdown includes tier label."""
        symbol_table = SymbolTable(file_path="src/auth.py", language="python")
        graph_constraints = GraphConstraints(file_path="src/auth.py", depth=5)
        ...
        assert "ENTERPRISE" in markdown or "enterprise" in markdown
