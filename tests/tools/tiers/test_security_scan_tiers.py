...
async def test_security_scan_community_enforces_finding_cap(community_tier):
    ...
    # Note: Each eval() call generates 2 vulnerabilities (pattern + taint)

    code = _make_repetitive_vuln_code(30)  # 30 * 2 = 60 vulns, capped at 50
    ...
    assert result.vulnerability_count == 50  # Community cap
...
async def test_security_scan_pro_allows_unlimited_findings(pro_tier):
    ...
    code = _make_repetitive_vuln_code(60)  # 60 * 2 = 120 vulns

    result = await security_scan(code=code)

    assert result.success is True
    assert result.vulnerability_count == 120  # 60 calls * 2 detections
    assert len(result.vulnerabilities) == 120
...
async def test_security_scan_enterprise_enables_enterprise_fields(enterprise_tier):
    ...
    # Test code that triggers vulnerabilities and enrichments
    code = """\
import hashlib
import os
...
async def test_security_scan_missing_license_defaults_to_community(community_tier):
    ...
    code = _make_repetitive_vuln_code(5)  # 5 * 2 = 10 vulns
    ...
    assert result.success is True
    assert result.vulnerability_count == 10  # 5 calls * 2 detections
