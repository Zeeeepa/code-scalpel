...
class TestLicenseFallback:
    ...
    async def test_expired_license_fallback(self, server_expired_license, project_120_files):
        ...
        assert hasattr(result, 'warnings') and result.warnings, \
            "Expected warnings about expired license"
        
        assert any('expired' in str(w).lower() for w in result.warnings), \
    ...
    async def test_invalid_license_fallback(self, server_invalid_license, project_120_files):
        ...
        assert hasattr(result, 'warnings') and result.warnings, \
            "Expected warnings about invalid license"
        
        assert any('invalid' in str(w).lower() for w in result.warnings), \
    ...
    async def test_valid_pro_license_unlocks_features(self, community_server, pro_server, simple_project):
        ...
        assert not has_pro_features(com_result), \
        ...
        assert len(available) > 0, \
            f"Pro tier should have Pro feature fields. Available: {list(pro_dict.keys())}"
