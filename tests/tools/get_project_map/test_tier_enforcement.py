...
class TestTierTransitions:
    ...
    async def test_community_to_pro_upgrade(self, community_server, pro_server, project_120_files):
        ...
        com_result = await community_server.get_project_map(
            project_root=str(project_120_files),
            include_complexity=False
        )
        
        assert com_result.total_files <= 100, \
        ...
        assert pro_result.total_files >= 100, \
            f"Pro should handle >100 files, got {pro_result.total_files}"
        assert pro_result.total_files >= com_result.total_files, \
    ...
    async def test_pro_to_enterprise_features(self, pro_server, enterprise_server, simple_project):
        ...
        assert not has_enterprise_features(pro_result), \
    ...
    @pytest.mark.asyncio
    async def test_tier_detail_level_progression(self, community_server, pro_server, 
                                                  enterprise_server, simple_project):
