...
class TestCommunityTierRealEnforcement:
    ...
    async def test_community_tier_10_update_limit(self, monkeypatch, community_tier, tmp_path):
        ...
        for i in range(10):
            ...
            assert result.success is True, f"Update {i+1} failed: {result.error if not result.success else ''}"
    ...
    @pytest.mark.asyncio
    async def test_community_excludes_pro_fields(self, monkeypatch, community_tier, tmp_path):
...
class TestTierFallbackBehavior:
    ...
    async def test_invalid_license_falls_back_to_community(self, monkeypatch, tmp_path):
        ...
        for i in range(11):
            ...
            if i < 10:
                assert result.success is True, f"Update {i+1} should succeed (Community allows up to 10)"
            else:
