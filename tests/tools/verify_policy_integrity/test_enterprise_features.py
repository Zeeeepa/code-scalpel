...
def test_batch_verification_performance_with_many_files(
    ...
    assert elapsed_time < 10.0, (
        f"Batch verification took {elapsed_time:.2f}s, "
        f"expected <10s (roadmap target: <5s for 200 files)"
    )
...
def test_enterprise_vs_pro_feature_matrix(
    ...
    print(f"   Community: basic_verification only")
    print(f"   Pro:       + signature_validation + tamper_detection")
    print(f"   Enterprise: + audit_logging (full_integrity_check)")
